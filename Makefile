...
EMUCMD := echo "Opening $(PROGRAM).dsk" && open $(PROGRAM).dsk && echo 
...
PREEMUCMD := cp ./assets/$(PROGRAM)-boot.dsk ./$(PROGRAM).dsk &&\
java -jar $(AC) -d $(PROGRAM).dsk $(PROGRAM) &&\
java -jar $(AC) -as $(PROGRAM).dsk $(PROGRAM) BIN < $(PROGRAM).apple2enh
POSTEMUCMD :=
